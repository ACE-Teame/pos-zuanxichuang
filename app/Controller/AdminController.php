...
class AdminController extends Wb_Controller
{
    ...
    public function orderList()
    {
        ...
        foreach ($data['orderData'] as $key => $value) {
            $province_name = parent::$model->select('province', 'province_name', ['id' => $value['province_id']])[0];
            $city_name = parent::$model->select('city', 'city_name', ['id' => $value['city_id']])[0];
            $data['orderData'][$key]['address'] = $province_name . ' ' . $city_name;
        }
    ...
    public function downloadOrder()
    {
        ...
        foreach ($orderData as $key => $value) {
            $province_name = parent::$model->select('province', 'province_name', ['id' => $value['province_id']])[0];
            $city_name = parent::$model->select('city', 'city_name', ['id' => $value['city_id']])[0];
            $orderData[$key]['address'] = $province_name . ' ' . $city_name;
        }

        echo "\xEF\xBB\xBF子链接,用户名,电话,地址,提交时间\r";
        ob_end_flush();  
        foreach($orderData as $order) {  
            echo $order['c'] . "," . "\"\t" . $order['username'] . "\",\"\t" . $order['phone'] . "\",\"\t" . $order['address'] ."\",\"\t" .  get_date($order['time']). "\"\t\r";  
            flush();  
        }  
    }
}